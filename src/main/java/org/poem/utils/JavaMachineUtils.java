...
public class JavaMachineUtils {
    ...
    public static JavaClass buildClass(Class clazz) {
        JavaLoaderMachine javaLoaderMachine = new JavaLoaderMachine(clazz);
        return javaLoaderMachine.build();
    }
    ...
    public static List<org.poem.lang.core.Annotation> annotationsType(Annotation[] clazzs) {
        if (null != clazzs && clazzs.length > 0) {
            return  Lists.asList(clazzs).stream().map(new AnnotationFunction()).collect(Collectors.toList());
        }
        return null;
    }
}
